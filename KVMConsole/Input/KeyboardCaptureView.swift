...
final class CaptureNSView: NSView {
    ...
    private func updateMouseMovedDelivery() {
        guard let window else { return }
        if isMouseEnabled {
            if savedAcceptsMouseMovedEvents == nil {
                savedAcceptsMouseMovedEvents = window.acceptsMouseMovedEvents
            }
            window.acceptsMouseMovedEvents = true
        } else if let savedAcceptsMouseMovedEvents {
            window.acceptsMouseMovedEvents = savedAcceptsMouseMovedEvents
            self.savedAcceptsMouseMovedEvents = nil
        }
    }
